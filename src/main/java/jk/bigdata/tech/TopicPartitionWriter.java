...
public class TopicPartitionWriter {
  ...
  private final RecordWriterProvider<AdlSinkConnectorConfig> writerProvider;
  ...
  private final AdlSinkConnectorConfig connectorConfig;
  ...
  public TopicPartitionWriter(TopicPartition tp,
                              ...
                              AdlSinkConnectorConfig connectorConfig,
  ...
  TopicPartitionWriter(TopicPartition tp,
    ...
    this.timestampExtractor = partitioner instanceof TimeBasedPartitioner
            ...
            : null;
    flushSize = connectorConfig.getInt(AdlSinkConnectorConfig
            .FLUSH_SIZE_CONFIG);
    topicsDir = connectorConfig.getString(StorageCommonConfig
            .TOPICS_DIR_CONFIG);
    rotateIntervalMs = connectorConfig.getLong(AdlSinkConnectorConfig
            .ROTATE_INTERVAL_MS_CONFIG);
    rotateScheduleIntervalMs =
            connectorConfig.getLong(AdlSinkConnectorConfig
                    .ROTATE_SCHEDULE_INTERVAL_MS_CONFIG);
    if (rotateScheduleIntervalMs > 0) {
      timeZone = DateTimeZone.forID(connectorConfig.getString
              (PartitionerConfig.TIMEZONE_CONFIG));
    }
    timeoutMs = connectorConfig.getLong(AdlSinkConnectorConfig
            .RETRY_BACKOFF_CONFIG);
    compatibility = StorageSchemaCompatibility.getCompatibility(
            connectorConfig.getString(HiveConfig
    ...
    failureTime = -1L;
    currentOffset = -1L;
    dirDelim = connectorConfig.getString(StorageCommonConfig
            .DIRECTORY_DELIM_CONFIG);
    fileDelim = connectorConfig.getString(StorageCommonConfig
            .FILE_DELIM_CONFIG);
    extension = writerProvider.getExtension();
    zeroPadOffsetFormat = "%0"
            + connectorConfig.getInt(AdlSinkConnectorConfig
  ...
  private void executeState(long now) {
    switch (state) {
      ...
      case WRITE_PARTITION_PAUSED:
        ...
        Schema currentValueSchema = currentSchemas.get
                (encodedPartition);
      ...
      case FILE_COMMITTED:
        ...
        break;
      default:
        log.error("{} is not a valid state to write record for topic " +
                "partition {}.", state, tp);
    }
  }
  ...
  private boolean checkRotationOrAppend(
    ...
    if (compatibility.shouldChangeSchema(record, null, currentValueSchema)
            && recordCount > 0) {
      // This branch is never true for the first record read by this
      // TopicPartitionWriter
      log.trace(
              "Incompatible change of schema detected for record '{}' " +
                      "with encoded partition "
                      + "'{}' and current offset: '{}'",
              record,
              encodedPartition,
              currentOffset
  ...
  private void commitOnTimeIfNoData(long now) {
    if (buffer.isEmpty()) {
      // committing files after waiting for rotateIntervalMs time but
      // less than flush.size
      // records available
      if (recordCount > 0 && rotateOnTime(currentEncodedPartition,
              currentTimestamp, now)) {
        log.info(
                "Committing files after waiting for rotateIntervalMs " +
                        "time but less than flush.size "
                        + "records available."
        );
        setNextScheduledRotation();
  ...
  private String getDirectoryPrefix(String encodedPartition) {
    return partitioner.generatePartitionedPath(tp.topic(),
            encodedPartition);
  }
  ...
  private boolean rotateOnTime(String encodedPartition, Long
          recordTimestamp, long now) {
    if (recordCount <= 0) {
      return false;
    }
    ...
    log.trace(
            "Checking rotation on time with recordCount '{}' and " +
                    "encodedPartition '{}'",
            recordCount,
            encodedPartition
    );
    ...
    log.trace(
            "Should apply periodic time-based rotation (rotateIntervalMs:" +
                    " '{}', baseRecordTimestamp: "
                    + "'{}', timestamp: '{}')? {}",
            rotateIntervalMs,
            baseRecordTimestamp,
    ...
    boolean scheduledRotation = rotateScheduleIntervalMs > 0 && now >=
            nextScheduledRotation;
    log.trace(
            "Should apply scheduled rotation: (rotateScheduleIntervalMs: " +
                    "'{}', nextScheduledRotation:"
                    + " '{}', now: '{}')? {}",
            rotateScheduleIntervalMs,
            nextScheduledRotation,
  ...
  private void setNextScheduledRotation() {
    if (rotateScheduleIntervalMs > 0) {
      ...
      if (log.isDebugEnabled()) {
        log.debug(
                "Update scheduled rotation timer. Next rotation for " +
                        "{} will be at {}",
                tp,
                new DateTime(nextScheduledRotation).withZone
                        (timeZone).toString()
        );
      }
    }
  ...
  private RecordWriter getWriter(SinkRecord record, String encodedPartition)
    ...
    RecordWriter writer = writerProvider.getRecordWriter(connectorConfig,
            commitFilename);
  ...
  private void commitFiles() {
    ...
    log.info("Files committed to Azure blob. Target commit offset for {} is {}",
            tp, offsetToCommit);
  }

  private void commitFile(String encodedPartition) {
    if (!startOffsets.containsKey(encodedPartition)) {
      log.warn("Tried to commit file with missing starting offset partition: " +
              "{}. Ignoring.");
      return;
    }
